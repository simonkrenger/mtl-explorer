...
- Required previous coverage IDs or run packets: GPS_01 terminal.
...
|---|---|---|---|---|---|
| GPS_02 | Attempted to enable GPS on the configured target by clicking the GPS control. | On a secure origin, the browser permission prompt can be accepted and the locate marker should appear. On remote plain HTTP, the row is not applicable per GPS_01 coverage guidance. | No usable permission prompt was available for live GPS because the target was a remote plain-HTTP origin with `window.isSecureContext=false`; clicking GPS showed the secure-origin warning instead of starting a GPS watch or marker. | NOT APPLICABLE | [assets/GPS-geolocation-insecure-context.webp](../assets/GPS-geolocation-insecure-context.webp); [assets/GPS-geolocation-insecure-context.txt](../assets/GPS-geolocation-insecure-context.txt) |
