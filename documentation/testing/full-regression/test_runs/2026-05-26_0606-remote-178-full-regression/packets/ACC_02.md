...
- In scope: Do not pass parent areas when children are skipped.
...
|---|---|---|---|---|---|
| ACC_02 | Reviewed queue/accounting requirement and current run-state/report workflow. | Coverage is tracked per stable ID with explicit evidence and terminal status rules. | Using one packet per child coverage ID; parent/section summaries will not substitute for child statuses. | PASS | run-state.md |
