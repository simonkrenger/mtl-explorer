...
- In scope: Report coverage detail.
...
|---|---|---|---|---|---|
| ACC_03 | Reviewed queue/accounting requirement and current run-state/report workflow. | Coverage is tracked per stable ID with explicit evidence and terminal status rules. | Final report will be assembled from packet files only and include one row per coverage ID. | PASS | packets/*.md; report.md pending final gate |
