...
- **DAT_08** Use at least four fully synthetic, non-private photos with GPS EXIF
  metadata. Generate the standard JPEG set with the packaged
  `/app/demo/generate_regression_photos.py` helper. It uses the same shared
  renderer as the demo photos. The helper produces four GPS photos, two
  camera-time-only photos, and a matching six-point synthetic GPX activity.
  Record the generated manifest. Copy the GPX into `data/gpx/<run-id>/` and the
  six JPEGs into `data/media/<run-id>/`; keep the manifest and deletion backups
  outside the watched media tree.
...
- **MED_06** Capture the media count, MEDIA indexer status, and media freshness
  ...
  Place all six generated JPEGs in the watched media folder, open Admin, trigger
  **Rescan Media**, and wait for MEDIA indexing and correlation to settle.
  Accept the freshness reload and verify all six files are completed without
  failures. Preserve the generated manifest and exact source files outside the
  watched tree for later deletion evidence and recovery.
...
| Data-change | `DAT`, `IMP`, `FIT`, `DEL`, `MED_06-MED_33`, and `SYN`: import, conversion, track and photo deletion flows, persisted photo correlation, freshness, cache refresh, map, browser, statistics, responsive viewer, large-library paging, and media audit. |
